...
#define enumeratekt(ht,k,e,t,f,b) loopi((ht).size)  for(hashtable<k,t>::chain *enblue = (ht).table[i]; enblue; enblue = enblue->next) { hashtable<k,t>::const_key &e = enblue->key; t &f = enblue->data; b; }
#define enumerate(ht,t,e,b)       loopi((ht).size) for((ht).enblue = (ht).table[i]; (ht).enblue; (ht).enblue = (ht).enblue->next) { t &e = (ht).enblue->data; b; }
